...
void HealthSampler::sample_output(const Endpoint &ep, const std::shared_ptr<OutputController> &ctrl)
{
	...
	snap.last_error     = ctrl ? ctrl->last_error() : "";
	snap.state          = ctrl ? ctrl->state() : OutputState::Idle;

	if (!ctrl || snap.state == OutputState::Idle || snap.state == OutputState::FailedHard) {
		/* Zero out stats for non-running endpoints */
